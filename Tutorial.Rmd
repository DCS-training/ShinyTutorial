...
## Why creating own web apps (and why in Shiny)? {#why}

|
| 
Sometimes a raw code, accompanied by pre-generated tables and visualizations, is not enough to effectively convey the meaning of one's work. Various types of output files are by design static, showcasing results but not analysis behind them. In turn, code itself, a step-by-step description of an analysis, is generally hard to effectively present and explain to non-coding colleges or a conference attendees. Both approaches also reduce any interaction with a wider public down to passive participation, often discouraging to the even most dedicated audience.
...
R can be successfully used to write apps with the aid of Shiny. Shiny is a little bit more just a library of dedicated functions, it is a whole framework of solutions for creating, running, sharing and deploying apps. While sounding complex, it is perhaps the easiest way to experience the entry-level of app development. Shiny app structure is surprisingly simple yet insanely flexible, allowing applications as complex as user wants them to be. Knowledge of HTML, CSS or JavaScript, basic languages used when building an app, is not a necessity. Everything can be written in R, with Shiny then translating the code into relevant languages on its own.

If you already know how to code in R, why not to take a step further?
...
Due to how important theory is to understand the inner structure and workings of a web app,  the tutorial proper starts with a short description of key web app features and how they relate to each other ([*How web applications work?*](#howappswork)). Then the tutorial proceeds with explaining how the general app structure is reflected within a Shiny framework ([*Shiny: app structure*](#appstructure)) and some fundamental customization options ([*Shiny: app themes*](#customization); [*Shiny: writing in-app*](#customization2)). Once the basic structure is understood, the tutorial proceeds with explaining each stage of creating an app, from designing a user interface ([*Shiny: UI, designing layout*](#designinglayout)) and getting user input ([*Shiny: UI, getting user input*](#gettinginput)), through processing input to get output ([*Shiny: server and rendering output*](#renderingoutput)) and displaying it on the user interface ([*Shiny: UI, displaying output*](#displayingoutput)), ending on discussing an example of a completed, functional app ([*Shiny: a functional example*](#functionalexample)).

|
The tutorial ends with a brief discussion on key venues for sharing or publishing a shiny app ([*Sharing/deploying an app*](#Publishing)).If by the end  you will be interested in mastering Shiny to a greater degree, see [*Closing remarks*](#remarks) and [*Further reading and external resources*](#furtherreading) pages. The first one shortly discuss possible venues for developing your Shiny skills further. The second one includes links to all the relevant sites, including tutorials of various degrees of complexity, presentations and reference sheets summarizing key points, ending with books with in-depth knowledge about building complex Shiny applications and hosting them online.
...
```
|
If you decide with creating a new file through *Shiny Web App...* you will be first asked where you wish to create an app in one or multiple R files. The reason behind the prompt will be answered early in this tutorial, but of now I suggest starting with just one file. Once the option is selected, a new app is named and its location defined, a new file will be created. It will contain an example code for a very simple app, can be either completely erased or repurposed.
...
## How web applications work? {#howappswork}
...
Despite web applications coming in all shapes or forms, all are sharing the same structure. 
...
```{r fig.align="center", fig.show="hold",out.width = '100%', fig.cap = "Written text displayed in a Shiny ap", echo=FALSE}
knitr::include_graphics("visual/TextExamples.jpg")
```
| 
| 
Sometimes you may want to put a written text in the application itself, for example a reason behind the app or a basic outline how to use it. 
...
If you wish for some of the options to remain hidden till the specific conditions occur, you can use *conditionalPanel()* function. two key arguments to include is the *conditon*, under which additional widget will appear, and the widget itself. For example, *conditon = ("input.InputName = 2")* tells us, that the specific input, here named *InputName*, needs to be set as numerical 2 to trigger the additional widget to appear. One thing to remember though, is that conditions are evaluated under the JavaScript coding, not R.
...
#Example of using a conditional panel
  ui <- fluidPage(
    sidebarLayout(
      ...
      ,  
      conditonalPanel(              
          conditon = (               #Establishing a conditon
...
|
| 
So, how the final application looks like when we combine all the elements? And how the whole thought process looks like, from a key idea?

You can find the example app code here, after the app explanation, as well as load the app from your RStudio by typing *shiny::runGitHub("ShinyTutorial","DCS-training", ref= "main/appexample")* in your console. 
...
|
| 
So you wrote an app, what now? Well, Shiny app can be either shared or deployed. By *sharing* we understand simply providing means of downloading r script and related data. The receiving user needs to have RStudio and all the required libraries installed to run an app locally. In turn, *deployment* means putting your app, with all dependencies (required files and data), on an online server, accessible through a unique URL. Once completed, the app is running fully on an online server, with a receiving user only needing an URL to access it. 
...
| 
```{r fig.align="center", fig.show="hold", echo=FALSE,  fig.cap = "Shinyapps.io user panel fro a shiny app", out.width = '90%'}
...
| [Official tutorials (begginer)](https://shiny.rstudio.com/tutorial/)
| [*a gRadual intRoduction to Shiny* (Begginer)](https://laderast.github.io/gradual_shiny/)
...
| [LinkedIn Learning: *Building data apps with R and Shiny: Essential Training* (Begginer)](https://www.linkedin.com/learning/building-data-apps-with-r-and-shiny-essential-training/build-test-and-deploy-apps-easily-in-shiny?u=50251009)
| [LinkedIn Learning: *Creating interactive presentations with Shiny and R* (Begginer)](https://www.linkedin.com/learning/creating-interactive-presentations-with-shiny-and-r/welcome?u=50251009)
| [DataCamp: *Shiny fundamentals with R* (Begginer)](https://www.datacamp.com/tracks/shiny-fundamentals-with-r?tap_a=5644-dce66f&tap_s=463826-784532&irclickid=SOUUtsxeLxyITXGxavSWgyt9UkBXtywdHQhF280&irgwc=1)
| [UDEMY, available Shiny courses (Begginer-Advanced)](https://www.udemy.com/topic/shiny/?ranMID=39197&ranEAID=vedj0cWlu2Y&ranSiteID=vedj0cWlu2Y-__lLdGo1NtaPge2GVjICig&LSNPUBID=vedj0cWlu2Y&utm_source=aff-campaign&utm_medium=udemyads)
...
| [Supplement to Shiny in Production (Begginer-Intermediate)](https://kellobri.github.io/shiny-prod-book/index.html)
| 
| **Books available online**
| [*Mastering Shiny* (Begginer-Intermediate)](https://mastering-shiny.org/)
| [*Mastering Shiny Solutions* (Begginer-Intermediate)](https://mastering-shiny-solutions.org/)
...
| *Web Application Development with R Using Shiny - Third Edition (Begginer-Intermediate)* (2018, C. Beeley & S.R. Sukhdeve, Packt Publishing Ltd)
| *Building Shiny Apps: Web Development for R users (English Edition) (Begginer-Intermediate)* (2017, P. Macdonaldo)
|
...
| PDF document itself was knitted in RStudio (ver. 1.3.1073; R ver. 4.02), using RMarkdown library (ver. 2.7) and TinyTex (LaTex distribution). Diagrams presented (Fig. 5-7) were done in MS PowerPoint....for some reason.
...
|
| 
Tutorial nad exemple app accessible on GitHub: [github.com/DCS-training/ShinyTutorial](https://github.com/DCS-training/ShinyTutorial)
| 
| Andrzej A. Romaniuk
...
| **[HCA, personal page](https://www.ed.ac.uk/profile/andrzej-romaniuk)**
| **[LinkedIn](https://www.linkedin.com/in/andrzej-aleksander-romaniuk-74145292/)**
